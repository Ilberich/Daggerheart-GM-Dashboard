...
- **Rules Reference** — searchable Daggerheart cheat sheet (60+ entries across 16 categories) with SRD-accurate rules for resting (short/long rest downtime moves, 3-short-rest limit, interrupted rest handling), death moves (Avoid Death scar roll timing), and more; add custom rules; click ✎ on any rule to override its text with homebrew content; click ↺ Restore Default to revert. Custom rules can be deleted from the edit form.
